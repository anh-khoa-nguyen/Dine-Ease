...
|:---:|---|---|---|---|---|
| **W1** | **Khởi động** | Setup GitHub, tạo cấu trúc thư mục, viết `README.md` (Proposal). | | Thảo luận chọn đề tài, thống nhất tính năng MVP. Nhận phân công công việc. | Đề xuất đề tài |
| **W2** | **Phân tích** | Vẽ Wireframes UI cho trang Admin. | | Phân tích yêu cầu, viết Use cases và vẽ Wireframes cho luồng Nhà hàng. | `docs/requirements.md` |
| **W3** | **Thiết kế** | Thiết kế Database (ERD), tạo file `database-design.md`. | | Thiết kế API Docs cho Nhà hàng (`api-docs.md`). | Database + API design |
| **W4** | **Backend Core** | Setup Spring Boot, cấu hình MySQL. Tạo Entities/Repos chung. | | Tạo Entities & Repositories cho Nhà hàng. | Setup project |
| **W5** | **Backend Core** | Viết Basic APIs (CRUD) cho Admin. | | Viết Basic APIs (CRUD) cho Nhà hàng. | Backend chạy được |
| **W6** | **Backend + Auth** | Cấu hình Spring Security (Login/Register/JWT). | | Xử lý logic Sơ đồ bàn & Duyệt đơn. | Code logic Backend |
| **W7** | **Backend + Auth** | Tích hợp **JavaMail** (Gửi email). | | Tích hợp **Cloudinary** (Upload ảnh). | APIs hoàn chỉnh |
| **W8** | **Frontend** | Setup ReactJS, code UI/UX trang Admin, gọi API Admin. | | Code UI/UX trang Nhà hàng, gọi API Nhà hàng. | Frontend cơ bản |
| **W9** | **Hoàn thiện** | **(QA/Tester)** Test chéo luồng thanh toán của Yến. Fix bugs Admin. | | **(QA/Tester)** Test chéo luồng gửi mail của Khoa. Fix bugs Nhà hàng. | Sản phẩm hoàn chỉnh |
| **W10** | **Bảo vệ** | Viết báo cáo tổng kết, làm Slide thuyết trình. | | Hoàn thiện API Docs, dọn dẹp source code. | Nộp bài + Bảo vệ |
## 7. Timeline
